layui.use(['form', 'layer', 'laydate', 'table', 'laytpl'], function () {
    ...
    var tableIns = table.render({
        elem: '#tb',
        url: '../supplier/querySupplierPage?type=2',
        cellMinWidth: 95,
        toolbar: '#toolbarDemo',
        page: true,
        limit: 10,
    ...
    $('#add_supplier').click(function () {
        layer.open({
            ...
            yes: function(index, layero){
                ...
                basesubmits("../supplier/addsupplier?type=2", $(from).serialize(),index);
    ...
    table.on('tool(supplier)', function(obj) {
        var data = obj.data,
            layEvent = obj.event;
        if (layEvent === 'del') {
            layer.confirm('确定删除客户 '+ data.name +  '?', {icon: 3, title: '提示信息'}, function (index) {
                var date = 'id=' + data.id + '';
            ...
            layer.open({
                ...
                yes: function(index, layero){
                    var from = layer.getChildFrame('body', index).find("#from_addsupplier");
                    basesubmits("../supplier/updatesupplier?type=2", $(from).serialize(),index);
                    //parent.layer.close(index); //再执行关闭，强制关闭弹窗
                },
                end: function () {//在关闭的时候刷新表单
                    window.location.reload();
